...
class FolderMonitor(FileSystemEventHandler):
    ...
    def update_model(self):
        ...
        persist_cluster_labels(cluster_labels,average_vectors)
...
def persist_cluster_labels(cluster_labels, documents):
    if len(cluster_labels) != len(documents):
        raise ValueError("Number of cluster labels and documents must match.")

    with open('cluster_data.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['Document', 'Label'])  # Header row

        for i, label in enumerate(cluster_labels):
            writer.writerow([documents[i], label])
...
def main():
    ...
    folder_path = os.path.join(project_root, 'Crawler', 'indexes')
